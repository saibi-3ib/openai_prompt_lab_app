...
def get_current_prompt(db: Session) -> Prompt:
    ...
    default_setting = (
        db.query(Setting).filter(Setting.key == "default_prompt_name").first()
    )
    prompt = None
    ...
    if not prompt:
        default_text = """
                        ...
                        """
        new_prompt = Prompt(
            name=DEFAULT_PROMPT_KEY, template_text=default_text, is_default=True
        )
        db.add(new_prompt)
        db.commit()
...
def _run_analysis_logic(
    ...
    target_account = (
        ...
        .filter(TargetAccount.username == username_to_process)
        .first()
    )
    ...
    current_prompt = (
        db.query(Prompt).filter(Prompt.name == selected_prompt_name).first()
    )
        ...
        try:
            ...
            response = client_openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a data extraction engine designed to output JSON.",
                    },
            ...
            for post_analysis in detailed_analysis:
                if post_analysis.get("post_db_id") != post.id:
                    print(
                        f"Warning: AI returned wrong post_db_id. Expected {post.id}, got {post_analysis.get('post_db_id')}"
                    )
                    continue
                ...
                for sentiment_data in ticker_sentiments:
                    ...
                    ticker_mention_counts[ticker] = (
                        ticker_mention_counts.get(ticker, 0) + 1
                    )
    ...
    total_cost = calculate_cost(
        selected_model,
        {"prompt_tokens": total_input_tokens, "completion_tokens": total_output_tokens},
    )
    ...
    new_result.raw_json_response = (
        f"Aggregated {len(posts)} posts. See TickerSentiment table for details."
    )

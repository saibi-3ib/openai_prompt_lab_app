...
def recalculate_all_weights():
    ...
    try:
        ...
        for account_id in account_ids:
            ...
            result = (
                db.query(func.sum(UserTickerWeight.total_mentions).label("total"))
                .filter(UserTickerWeight.account_id == account_id)
                .first()
            )
            ...
            db.query(UserTickerWeight).filter(
                ...
                {
                    # (例: 11 / 20 = 0.55)
                    "weight_ratio": UserTickerWeight.total_mentions
                    / float(user_total_mentions)
                },
                synchronize_session=False,
            )  # SQLAlchemyにキャッシュの同期をスキップさせる

            print(
                f"Successfully recalculated weights for Account ID {account_id} (Total Mentions: {user_total_mentions})"
            )
        ...
        db.commit()
        print(
            f"--- Weight recalculation batch finished. Processed {total_accounts_processed} accounts. ---"
        )
